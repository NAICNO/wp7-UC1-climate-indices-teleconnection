# Climate Indices Teleconnection Analysis Using Machine Learning and HPC

Analyze climate teleconnections between 65 climate indices using ML models on NAIC Orchestrator VMs and Sigma2 HPC clusters.
...
| `demonstrator.ipynb` | Full analysis notebook for Sigma2 SLURM clusters |
...
| XGBRegressor | Gradient boosting | Yes | Maximum accuracy with GPU |
...
├── demonstrator.ipynb                  # Full analysis notebook (SLURM)
...
├── utils.py                            # SLURM job submission utilities
...
```bash
tmux new-session -d -s training 'cd ~/d7.2-Use-case1 && source venv/bin/activate && \
python scripts/lrbased_teleconnection/main.py \
    --data_file dataset/noresm-f-p1000_slow_new_jfm.csv \
    --target_feature amo2 \
...
```bash
rsync -avz ubuntu@<VM_IP>:~/d7.2-Use-case1/results/ ./results/ -e "ssh -i <SSH_KEY_PATH>"
```
...
| Issue | Solution |
|-------|----------|
| `git: command not found` | `sudo apt install -y git` or `module load git` |
